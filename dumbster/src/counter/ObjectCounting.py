...
def main():
    print("[INFO] Lade Region-Konfiguration...")
    bbox = load_bbox()
    if not bbox:
        print("[ERROR] Kein Zählbereich definiert.")
        return

    # Skalierung von 720px Canvas auf 1280px Originalbild
    canvas_width = 720
    original_width = 1280
    ...
    print(f"[INFO] Region Points: {region}")

    print("[INFO] Initialisiere ObjectCounter...")
    counter = ObjectCounter(
        model=MODEL_PATH,
        classes=[0],  # nur Personen zählen
        region=region,
        stream=True,
        show=True  # Debug-Overlay aktiv
    )

    print("[INFO] Starte Zählung...")
    while True:
        try:
            if not os.path.exists(LIVE_FRAME_PATH):
                print("[WARN] live_frame.npy nicht gefunden. Warte...")
                time.sleep(0.2)
                continue

            # Robust gegen Race-Condition beim Laden von .npy
            frame = None
            ...
            if frame is None:
                print("[WARN] Frame konnte nicht gelesen werden. Frame wird übersprungen.")
                continue
            ...
            print(f"[ERROR] Fehler während der Verarbeitung: {e}")
            time.sleep(0.5)
